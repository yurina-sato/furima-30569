...
RSpec.describe 'ユーザー新規登録', type: :system do
  ...
  context 'ユーザー新規登録ができないとき' do
    it '誤った情報ではユーザー新規登録ができずに新規登録ページへ戻ってくる' do
      ...
      select "--", from: 'user_birth_date_1i' # 生"年"月日
      select "--", from: 'user_birth_date_2i' # 生年"月"日(0を抜いて左寄せ)
      select "--", from: 'user_birth_date_3i' # 生年月"日"(0を抜いて左寄せ)
...
RSpec.describe 'ログイン', type: :system do
  ...
  context 'ログインができるとき' do
    it '保存されているユーザーの情報と合致すればログインができる' do
      ...
      expect(page).to have_no_content('新規登録')
      expect(page).to have_no_link('ログイン'), href: "/users/sign_in" # flashメッセージの「ログインしました」と被るためリンクで指定
    end
  end
